  ...
  return (
    <div className="max-w-6xl mx-auto px-5 py-10" data-testid="results-page">
      ...
      <section className="grid md:grid-cols-2 gap-5 mt-6 fade-up delay-2">
        {data.eye_result && (
          <FindingsCard icon={<Eye className="w-4 h-4" />} title="Eye findings" result={data.eye_result} testId="findings-eye" />
        )}
        {data.nail_result && (
          <FindingsCard icon={<Hand className="w-4 h-4" />} title="Nail findings" result={data.nail_result} testId="findings-nail" />
        )}
      </section>

      {/* ── Nail clinical signs ── badge-style weights (Image 3 style) */}
      {data.nail_result?.model_extras?.clinical_features && (
        <section
          className="mt-6 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 fade-up delay-2"
          data-testid="nail-clinical-signs"
        >
          <div className="flex items-center gap-2 mb-1">
            <Hand className="w-4 h-4 text-[var(--primary)]" />
            <span className="font-heading font-medium">Nail · visual analyzer</span>
          ...
          <div className="grid sm:grid-cols-2 gap-x-8 gap-y-4">
            {Object.entries(data.nail_result.model_extras.clinical_features).map(([sign, v]) => {
              ...
              return (
                <div key={sign} className="py-1" data-testid={`nail-sign-${sign}`}>
                  <div className="flex items-center justify-between gap-2 text-sm">
                    {/* Label + weight badge */}
                    <div className="flex items-center gap-2 min-w-0">
                      <span className={positive ? "font-medium text-[var(--accent-urgent)] truncate" : "text-[var(--fg)] truncate"}>
                        {meta.label}
                      </span>
                      {/* CHANGED: pill badge instead of inline subscript */}
                      <span className="shrink-0 inline-flex items-center px-1.5 py-0.5 rounded-md bg-[var(--secondary)] text-[9px] font-medium text-[var(--muted)] tracking-wide whitespace-nowrap">
                  ...
                  {/* Progress bar */}
                  <div className="h-1.5 bg-[var(--surface-2)] rounded-full overflow-hidden mt-1.5">
                    <div
                  ...
                  <div className="text-[11px] text-[var(--muted)] mt-1 leading-snug">{meta.desc}</div>
                </div>
              );
            })}
          </div>

          {/* Blend explanation footer */}
          <div className="text-[11px] text-[var(--muted)] mt-4 pt-3 border-t border-[var(--border)]">
            ...
            <span className="text-[var(--fg)] font-mono">
              {Math.round((data.nail_result.model_extras.p_model_anemia || 0) * 100)}%
            </span>
            ...
            . Visual checks say:{" "}
            <span className="text-[var(--fg)] font-mono">
              {Math.round((data.nail_result.model_extras.p_features_anemia || 0) * 100)}%
            </span>
      ...
      {/* ── Grad-CAM viewers ── "attention map" variant */}
      <section className="grid lg:grid-cols-2 gap-5 mt-6 fade-up delay-3">
        {data.eye_image_base64 && (
          <GradCamViewer
            ...
            label="Eye · attention map"
            icon="eye"
            variant="attention-map"
          />
        )}
      ...
      <section className="mt-6 rounded-2xl border border-[var(--primary)]/30 bg-[var(--surface)] p-6 fade-up delay-4">
        ...
        <div className="mt-3 text-xs text-[var(--muted)]">
          Photos used in this estimate:{" "}
          <span className="text-[var(--fg)]">{data.fusion_result.modalities_used?.join(" + ") || "—"}</span>
        </div>
      </section>
...
function EmptyRisk({ title, subtitle, icon }) {
  return (
    <div className="rounded-2xl border border-dashed border-[var(--border)] bg-[var(--surface)] p-5 flex flex-col justify-center text-center">
      <div className="mx-auto w-9 h-9 rounded-lg bg-[var(--surface-2)] grid place-items-center text-[var(--muted)]">
        {icon}
      </div>
